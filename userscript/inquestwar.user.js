...
(function () {
    ...
    function parseTravel(status) {
        let m = status.match(/travel(?:l)?ing to (.+)/i);
        if (m) return { arrow: '>', country: m[1].trim() };
        m = status.match(/returning to torn from (.+)/i);
        if (m) return { arrow: '<', country: m[1].trim() };
        return null;
    }
    ...
    function formatStatus(m) {
        ...
        if (lower.includes('hospital')) {
            const hospTimer = formatHospTimer(m.until);
            return { label: hospTimer ? `Hospital ${hospTimer}` : 'Hospital', color: '#f66' };
        }
        if (lower.includes('federal')) {
            return { label: 'Federal', color: '#f66' };
        }
        if (lower.includes('okay')) {
            return { label: raw, color: '#4caf50' };
        }
        const travel = parseTravel(raw);
        if (travel) {
            return { label: `${travel.arrow} ${travel.country}`, color: '#8cf' };
        }
    ...
    function renderPanel() {
        ...
        const rows = sorted.map((m) => {
            ...
            const statusInfo = formatStatus(m);
            ...
            return `
                <tr style="border-bottom:1px solid #2a2a2a;">
                    <td style="padding:4px 2px;"><a href="https://www.torn.com/profiles.php?XID=${m.id}" target="_blank" style="color:#fff;text-decoration:none;">${escapeAttr(m.name)}</a></td>
                    <td style="padding:4px 2px;text-align:center;color:#fff;">${m.level}</td>
                    <td style="padding:4px 2px;color:${statusInfo.color};">${escapeAttr(statusInfo.label)}</td>
                    <td style="padding:4px 2px;text-align:center;"><a href="https://www.torn.com/page.php?sid=attack&user2ID=${m.id}" target="_blank" style="color:#7ec8ff;text-decoration:none;">Attack</a></td>
    ...
    function startPolling() {
        ...
        // fast local tick: auto-uncall check + countdown re-render for hospital timers, no network
        setInterval(() => {
            checkAutoUncall();
